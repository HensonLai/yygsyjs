...
function appStart(){
    ...
    function createRole(callback){
        ...
        $(".s2 menu ul").each(function(i){
            i += 1;
            level = parseInt($('.s2 figure .item'+i).css('z-index')) || 0;
        ...
        body = window.imgCache[$(".s2 figure").is('.role2') ? 'role2.png' : 'role1.png'];
                ...
                if(app.poster.exCost[imgID]){
                    $.each(app.poster.exCost[imgID], function(i,o) {
                        ...
                        if(about.children('li.curr').hasClass('off')){
                            $(".s2 figure .item"+i).css('background-image', 'none');
